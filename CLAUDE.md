...
- The Bryant St Festival entry is `STOPS.push(...)`-ed separately and styled with a 🎪 icon, pinned at Bryant St & N Euclid Ave with `zIndexOffset:1000` so it stacks above house pins. Its lineup lives near the top of the script — `FESTIVAL_SCHEDULE` (`{ time, act }`), `FOOD_TRUCKS` (`{ name, url? }`, linkified when url present), and `VENDOR_OVERVIEW` (an HTML blurb; the owner explicitly does NOT want the full vendor list replicated, and no individual vendor highlights either — short overview only). All rendered by the festival modal (`openFestival()`), reachable from the 🎪 popup and the welcome modal.
